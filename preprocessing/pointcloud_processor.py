...
class RotationFunctions(FunctionGenerator):
    def __init__(self, operator, inplace):
        ...
        self.inplace = inplace
        assert (
            self.operator.bmm(self.operator.transpose(1, 2).contiguous()).sum().item()
            - (operator.size(0) * operator.size(2))
        ) ** 2 < 0.001, "Input matrix is not a rotation matrix"
...
class Operation(object):
    ...
    def scale(self, scale_vector):
        scaling_op = ScaleFunctions(
            scale_vector.to(self.device).type(self.type), inplace=self.inplace
        )
    ...
    def translate(self, translation_vector):
        translation_op = TranslationFunctions(
            translation_vector.to(self.device).type(self.type), inplace=self.inplace
        )
    ...
    def rotate(self, rotation_vector):
        rotation_op = RotationFunctions(
            rotation_vector.to(self.device).type(self.type), inplace=self.inplace
        )
...
class Normalization(Operation):
    ...
    def normalize_unitL2ball(self):
        ...
        scaling_factor_square, _ = torch.max(
            torch.sum(self.points ** 2, dim=2, keepdim=True), dim=1, keepdim=True
        )
...
class DataAugmentation(Operation):
    ...
    def random_anisotropic_scaling(self, min_val=0.75, max_val=1.25):
        ...
        """
        scale = (
            torch.rand(self.points.size(0), 1, self.points.size(2))
            * (max_val - min_val)
            + min_val
        )
        self.scale(scale)
    ...
    def get_random_rotation_matrix(batch_size=1):
        ...
        scaling_factor_square, _ = torch.max(
            torch.sum(x ** 2, dim=2, keepdim=True), dim=1, keepdim=True
        )
        scaling_factor = torch.sqrt(scaling_factor_square)
        x /= scaling_factor
        x = x.squeeze()
        ...
        rotation_matrix[:, 0, 1].copy_(
            XX[:, 0, 1] * (1 - cos_angle) - x[:, 2] * sin_angle
        )
        rotation_matrix[:, 1, 0].copy_(
            XX[:, 0, 1] * (1 - cos_angle) + x[:, 2] * sin_angle
        )

        rotation_matrix[:, 0, 2].copy_(
            XX[:, 0, 2] * (1 - cos_angle) + x[:, 1] * sin_angle
        )
        rotation_matrix[:, 2, 0].copy_(
            XX[:, 0, 2] * (1 - cos_angle) - x[:, 1] * sin_angle
        )

        rotation_matrix[:, 1, 2].copy_(
            XX[:, 1, 2] * (1 - cos_angle) - x[:, 0] * sin_angle
        )
        rotation_matrix[:, 2, 1].copy_(
            XX[:, 1, 2] * (1 - cos_angle) + x[:, 0] * sin_angle
        )
    ...
    def random_rotation(self, normals=False):
        ...
        :return: torch Tensor Batch, N_pts, D_dim
        """
        rot_matrix = DataAugmentation.get_random_rotation_matrix(
            batch_size=self.points.size(0)
        )
        if normals:
            rot_matrix = torch.cat([rot_matrix, rot_matrix], dim=2)
        self.rotate(rot_matrix)
    ...
    def random_translation(self, scale=0.03):
        ...
        """
        translation_vector = (
            torch.rand(self.points.size(0), 1, self.points.size(2)) * 2 * scale - scale
        )
    ...
    def random_flips(self, dims=[]):
        ...
        scale_factor = (
            torch.randint(2, (self.points.size(0), 1, self.points.size(2))) * 2 - 1
        )
